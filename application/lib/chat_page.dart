...
// Function to send the user's prompt to the backend
  Future<Map<String, dynamic>> _sendToApi(
    ...
    final url = 'http://172.20.10.2:8080/chat';
  ...
  // Method to pick an image from gallery or take a new one
          ...
          actions: [
            TextButton(
              ...
              onPressed: () async {
                ...
                if (image != null) {
                  // Do something with the image (e.g., send it in chat)
                  File selectedImage = File(image.path);
                  print('Selected Image: ${selectedImage.path}');
                  // Here you can handle the image, like sending it as a message
                }
  ...
  // Handle picking and uploading PDF
  void _pickPdf() async {
    ...
    if (result != null && result.files.isNotEmpty) {
      ...
      await Future.delayed(
          Duration(seconds: 2)); // Simulate a delay for the upload process

      setState(() {
        _isLoading = false; // Set loading state to false
      });

      // Add logic to upload or process the selected file here
    } else {
